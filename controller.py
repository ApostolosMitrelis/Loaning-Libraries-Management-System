...
class LibraryController:
    ...
    def setup_admin_dashboard(self):
        ...
        buttons = [
            ...
            ("Διαχείριση Αξιολογήσεων", lambda: print("TODO: Reviews")),
            ("Διαχείριση Κατηγοριών", lambda: print("TODO: Category"))
        ]
    ...
    def make_reservation(self, isbn):
    ...
    def show_book_details(self):
        ...
        self.view.build_books_info_window(self.root, book_info, copies, lambda: self.make_reservation(isbn), ebook_callback)
    ...
    def show_space_reservation(self):
        ...
        columns = ["ID", "Χώρος", "Βιβλιοθήκη", "Ημερομηνία", "Ώρες", "Χαρακτηριστικά"]
        self.my_res_tree, _ = self.view.create_treeview(self.my_reservations_tab, columns, widths=[50, 200, 100, 200, 150])
    ...
    def cancel_selected_reservation(self):
        ...
        reservation_id = self.my_res_tree.item(selected[0])['values'][0]
        
        confirm = self.view.ask_confirmation("Επιβεβαίωση Ακύρωσης", "Είστε σίγουροι ότι θέλετε να ακυρώσετε αυτή την κράτηση;")
        
        if confirm:
            success, message = self.db.cancel_space_reservation(reservation_id)
            if success:
                self.view.show_message("Επιτυχία", message)
                self.load_my_reservations()
            else:
