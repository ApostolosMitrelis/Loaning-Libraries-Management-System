...
class LibraryModel:
    ...
    def fetch_one_dict(self, query: str, params: tuple = ()) -> Optional[dict]:
        """Fetch one row ως dictionary"""
        result = self.execute_query(query, params, fetch_one=True)
        return dict(result) if result else None

    def fetch_all_dict(self, query: str, params: tuple = ()) -> List[dict]:
        """Fetch all rows ως list of dictionaries"""
        results = self.execute_query(query, params, fetch_one=False)
        return [dict(row) for row in results]

    def execute_with_commit(self, query: str, params: tuple = ()) -> Tuple[bool, str]:
        try:
    ...
    def browse_all_books(self, category: str = "Όλες", language: str = "Όλες", libraries: str = "Όλες", search_term: str = "") -> List[dict]:
    ...
    def get_available_copies(self, isbn: str, library_id: int = None) -> List[dict]:
    ...
    def check_ebook_availability(self, isbn: str) -> Optional[int]:
    ...
    def create_ebook_loan(self, member_id: int, ebook_id: int) -> Tuple[bool, str]:
    ...
    def create_reservation(self, member_id: int, isbn: str) -> Tuple[bool, str]:
    ...
    def get_member_loans(self, member_id: int) -> List[dict]:
        ...
        query = """
            ...
            WHERE δ.ID_Μέλους = ? AND δ.Κατάσταση IN ('Ενεργός', 'Εκπρόθεσμος')
            ORDER BY δ.Ημερομηνία_Λήξης
            """
        return self.fetch_all_dict(query, (member_id,))

    def get_member_fines(self, member_id: int) -> List[dict]:
    ...
    def rate_book(self, member_id: int, isbn: str, rating: int, review: str = None) -> Tuple[bool, str]:
        """Αξιολόγηση βιβλίου"""
        today = datetime.now().strftime('%Y-%m-%d')
        success, msg = self.execute_with_commit(
            "INSERT INTO Αξιολόγηση (ID_Μέλους, ISBN, Βαθμολογία, Κείμενο, Ημερομηνία) VALUES (?, ?, ?, ?, ?)",
    ...
    def get_member_reservations(self, member_id: int) -> List[dict]:
    ...
    def get_member_loan_history_books(self, member_id: int) -> List[dict]:
        """Ανάκτηση βιβλίων που έχει δανειστεί το μέλος (για αξιολόγηση)"""
        query = """
            SELECT DISTINCT τ.ISBN, τ.Τίτλος, τ.Συγγραφέας
            FROM Δανεισμός δ
            JOIN Αντίτυπο α ON δ.ID_Αντιτύπου = α.ID_Αντιτύπου
            JOIN Τεκμήριο τ ON α.ISBN = τ.ISBN
            WHERE δ.ID_Μέλους = ?
    ...
    def get_member_ratings(self, member_id: int) -> List[dict]:
    ...
    def get_available_spaces(self, library_id: int = None, has_computers: bool = None, has_projector: bool = None, has_board: bool = None, has_ac: bool = None, has_printer: bool = None, has_sockets: bool = None) -> List[dict]:
    ...
    def check_space_availability(self, space_id: int, date: str, start_time: str) -> bool:
        try:
            fmt = "%H:%M"
    ...
    def create_space_reservation(self, member_id: int, space_id: int, date: str, time: str) -> Tuple[bool, str]:
    ...
    def get_member_space_reservations(self, member_id: int) -> List[dict]:
        query = '''SELECT Μέλος_Κάνει_Κράτηση_Χώρου.*, Χώρος_Μελέτης.Όνομα_Χώρου, Χώρος_Μελέτης.Χωρητικότητα, Βιβλιοθήκη.Όνομα as Βιβλιοθήκη
                   FROM Μέλος_Κάνει_Κράτηση_Χώρου
    ...
    def cancel_space_reservation(self, reservation_id: int) -> Tuple[bool, str]:
        """Διαγραφή κράτησης χώρου"""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        try:
            ...
            conn.commit()
            conn.close()
            return True, "Η κράτηση χώρου διαγράφηκε επιτυχώς"
    ...
    def get_all_copies_for_isbn(self, isbn: str, library_id: int = None) -> List[dict]:
    ...
    def add_document(self, isbn: str, title: str, author: str = None, publisher: str = None, year: str = None, language: str = None, category_id: int = None, edition: int = None) -> Tuple[bool, str]:
    ...
    def add_copy(self, isbn: str, library_id: int, condition: str = 'Καλή') -> Tuple[bool, str]:
        """Προσθήκη αντιτύπου"""
        existing = self.fetch_one_dict("SELECT ISBN FROM Τεκμήριο WHERE ISBN = ?", (isbn,))
        if not existing:
    ...
    def delete_copy(self, copy_id: int) -> Tuple[bool, str]:
        """Διαγραφή αντιτύπου"""
        conn = self.get_connection()
        cursor = conn.cursor()
    ...
    def get_all_loans(self, search_term: str = "", status_filter: str = "", library_filter: int = None) -> List[dict]:
    ...
    def create_loan(self, member_id: int, copy_id: int, staff_library_id: int) -> Tuple[bool, str]:
    ...
    def get_all_fines(self, search_term: str = "", status_filter: str = "Όλα") -> List[dict]:
    ...
    def update_fine_status(self, fine_id: int, new_status: str) -> Tuple[bool, str]:
        """Αλλαγή κατάστασης προστίμου"""
        return self.execute_with_commit(
            "UPDATE Πρόστιμο SET Κατάσταση = ? WHERE ID_Προστίμου = ?",
            (new_status, fine_id))

    def impose_fine(self, loan_id: int, amount: float, reason: str = None) -> Tuple[bool, str]:
        """Επιβολή προστίμου"""
        conn = self.get_connection()
        cursor = conn.cursor()
    ...
    def search_members(self, search_term: str) -> List[dict]:
        query = '''SELECT Μέλος.*, Βιβλιοθήκη.Όνομα as Βιβλιοθήκη
                   FROM Μέλος
                   JOIN Βιβλιοθήκη ON Μέλος.ID_Βιβλιοθήκης = Βιβλιοθήκη.ID
                   WHERE Μέλος.Όνομα LIKE ? OR Μέλος.Επώνυμο LIKE ? OR Μέλος.Email LIKE ?
                   ORDER BY Μέλος.Όνομα, Μέλος.Επώνυμο
                   LIMIT 50'''
        return self.fetch_all_dict(query, (f'%{search_term}%', f'%{search_term}%', f'%{search_term}%'))
    ...
    def get_libraries_type(self) -> List[dict]:
    ...
    def browse_all_libraries(self, types: str = "Όλες", cities: str = "Όλες", couriers: str = "Όλοι", search_term: str = "") -> List[dict]:
    ...
    def add_library(self, data: dict) -> Tuple[bool, str]:
        return self.execute_with_commit(
    ...
    def update_library(self, lib_id: int, data: dict) -> Tuple[bool, str]:
        return self.execute_with_commit(
    ...
    def delete_library(self, lib_id: int) -> Tuple[bool, str]:
        # Έλεγχος συσχετίσεων
...
# ==================== ΔΙΑΧΕΙΡΙΣΗ ΜΕΛΩΝ ==================== #
    ...
    def browse_members(self, search_term="") -> List[dict]:
        query = """
            SELECT μ.*, β.Όνομα as Βιβλιοθήκη
            FROM Μέλος μ
    ...
    def add_member(self, data: dict) -> Tuple[bool, str]:
        return self.execute_with_commit(
    ...
    def update_member(self, member_id: int, data: dict) -> Tuple[bool, str]:
        return self.execute_with_commit(
    ...
    def browse_staff(self, search_term="") -> List[dict]:
        query = """
            SELECT π.*, β.Όνομα as Βιβλιοθήκη
            FROM Προσωπικό π
            JOIN Βιβλιοθήκη β ON π.ID_Βιβλιοθήκης = β.ID_Βιβλιοθήκης
    ...
    def update_staff(self, staff_id: int, data: dict) -> Tuple[bool, str]:
        return self.execute_with_commit(
            """UPDATE Προσωπικό 
    ...
    def delete_staff(self, staff_id: int) -> Tuple[bool, str]:
    ...
    def get_active_reservations_for_book(self, isbn: str) -> List[dict]:
